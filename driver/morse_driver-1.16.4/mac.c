...
/* Forward declaration for TSF sync functions */
 ...
 * for clock synchronization purposes. This captures ALL received frames
 * (data, management, beacons) to ensure updates on every packet.
 */
static void morse_capture_rx_tsf(const struct morse_skb_rx_status *hdr_rx_status)
{
	unsigned long flags;
			...
			struct morse_skb_rx_status *hdr_rx_status)
{
	...
	/* Capture TSF timestamp from DATA frames only (not management/beacons)
	 * Management frames may have stale TSF values causing sync instability
	 */
	if (skb->data && skb->len >= sizeof(struct ieee80211_hdr)) {
		hdr = (struct ieee80211_hdr *)skb->data;
